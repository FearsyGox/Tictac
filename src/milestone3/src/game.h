...
void playGame(int clientSocket) {
    char board[3][3];
    for (int i=0; i<9; i++) board[i%3][i/3] = ' ';
    ...
    while (true) {
        ...
        // Receive the move from the client player
        if (currentPlayer == 'X') {

            cout << "waiting for the player X..." << endl;
...
void playGame_(int clientSocket) {
